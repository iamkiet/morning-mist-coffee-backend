...
**A review is classified exactly once — a one-shot AI call, not a conversation** — `POST /product-reviews` calls `ReviewClassificationPort.classify()` synchronously in the same request (one Gemini call, no retry loop — this app has no queue infra), and that's the only time classification ever runs for a given review. The result routes the review:
```
