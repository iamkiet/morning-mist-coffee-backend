...
**Product categories are flat, not hierarchical** — `product_categories` has no `parentId` (dropped, was unused dead weight: every category in seed data and the admin UI had `parentId: null`, never actually exercised). `products_categories` is the M:N join between products and categories. `ProductCategoryRepo.list()` returns the flat array sorted by name. `SetProductCategoriesUseCase` is replace-all per product. `GET /api/v1/product-categories` is public (category names aren't sensitive, and the storefront needs the list to build its filter for anonymous visitors) — only `POST`/`PATCH`/`DELETE` are admin/staff-gated. Every product read path (`list`/`getById`/`getBySlug`/chat `items`/create response) also attaches `categoryIds` **and** `categoryNames` onto the `ProductDTO` via `attachCategories`/`attachCategoriesOne` ([src/application/product/attach-categories.ts](src/application/product/attach-categories.ts)), so a product's own category label is available without a second request.
...
```
A review has **at most one reply**, ever — `CreateProductReviewReplyUseCase` throws `ConflictError` if the review already has one. Only `admin`/`staff` can create it (`POST /api/v1/product-reviews/:reviewId/replies`, `requireRole(['admin','staff'])`) — there is no customer-facing reply endpoint; a customer who wants to say more just writes a new review, or asks the storefront **Chat** assistant (a completely separate feature, see below) rather than threading on an existing review. When the single admin reply is created while the review is `pending_reply`, the review is auto-transitioned straight to `resolved` in the same use case — no separate `PATCH /:id/status` call needed for the common case. Staff and admin can still move a review to any status by hand via `PATCH /:id/status`.

**Public visibility of a review is a status+category gate, not an admin toggle** — `GET /api/v1/product-reviews/product/:productId` (no auth) only returns rows where `category != 'spam' AND status IN ('auto_responded', 'resolved')`. Complaints and suggestions are shown publicly too, not just compliments — the storefront intentionally shows negative reviews once they've been through the classification/response pipeline, rather than only curating positive ones. A review sitting in `pending_reply` (low AI confidence, high severity, or the Gemini call failed) is invisible on the storefront until an admin/staff replies (auto-`resolved`) or sets its status by hand.
...
**Chat is RAG, not context dumping** — `SendChatMessageUseCase` embeds the customer's latest message with `MultimodalEmbeddingPort.embedQuery` (voice search uses `embedAudioQuery`; product indexing uses `embedDocument` — task type must match Gemini's asymmetric retrieval convention), retrieves the top 8 products via `findSimilarByVector` (the same pgvector index voice search uses), and injects only those into the system prompt. A `ChatFilterExtractionPort` (Gemini structured output) extracts an explicit price range, weight, and requested `quantity` from the question. Price/weight apply as a SQL `WHERE` alongside the vector search — vector similarity alone cannot express numeric thresholds — matching "product has at least one variant in range" (an `EXISTS` subquery against `product_variants`), not the product's minimum price. `quantity` (how many products the customer asked for, e.g. "cho tôi 1 sản phẩm") isn't a SQL filter — it caps the final `items` array in `AnswerQueryService.answer()` (`DEFAULT_RESULT_LIMIT = 6` when unstated), a hard guarantee independent of whether the model's reply text also respects the count. Fallback chain: vector → `ilike` keyword → most recent products, all price-filtered, so the assistant never answers with an empty catalogue. Before building the prompt, retrieved products are enriched with variant price + property values (`buildCatalogueProducts`) so the model still sees price/origin/roast even though `Product` itself no longer carries them. The system prompt (`chat-system.md`) also instructs the model to only recommend/name a product when the question is actually about finding/buying/comparing coffee — an off-topic message (greeting, store hours, thanks) gets a plain reply with no product mentioned, and since `items` is filtered down to only products the reply text names (`reply.includes(product.name)`, no fallback to "all retrieved" when nothing is mentioned), `items` comes back empty for those. If the final `chat.reply` call itself fails (quota, timeout, network), `SendChatMessageUseCase` catches it and returns a fixed Vietnamese apology string instead of throwing — retrieval already succeeded independently, so the client still gets `200` with real products, never a 502 from a downstream Gemini outage. Gemini is reached through `ChatPort` / `GeminiChatAdapter` — never call the SDK from a controller. System prompts live as versioned JSON in `src/prompts/` (`{promptName, version, template, variables}`), imported with `with { type: 'json' }` — do not inline prompt text back into adapter files.
