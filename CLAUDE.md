...
**Product categories are hierarchical** — `product_categories.parentId` self-references for a tree (e.g. `Đồ uống` → `Cà phê` → `Arabica`). `products_categories` is the M:N join. `ProductCategoryRepo.list()` returns the flat array; callers build the tree client-side. `SetProductCategoriesUseCase` is replace-all per product. `GET /api/v1/product-categories` itself is admin/staff-only (whole-file hook), so a public/customer caller can never fetch the category tree directly — instead every product read path (`list`/`getById`/`getBySlug`/chat `items`/create response) attaches `categoryIds` **and** `categoryNames` onto the `ProductDTO` via `attachCategories`/`attachCategoriesOne` ([src/application/product/attach-categories.ts](src/application/product/attach-categories.ts)), so FE can show a product's category label without needing admin access.
