...
2. **Tích hợp Trợ lý ảo AI tư vấn (Chatbot):** Ứng dụng mô hình Gemini 3.5 Flash để tương tác tự nhiên, tư vấn thực đơn trực tiếp cho khách hàng bằng tiếng Việt dựa trên danh mục sản phẩm có sẵn trong cơ sở dữ liệu (Context-aware RAG).
...
*   **Về mặt AI:** Tích hợp trực tiếp Google Gemini API sử dụng model `gemini-3.5-flash`.
...
*   **AI Integration:** `@google/generative-ai` SDK để giao tiếp trực tiếp với mô hình Gemini 3.5 Flash.
...
Hệ thống sử dụng mô hình **Gemini 3.5 Flash** do tính năng vượt trội về tốc độ phản hồi (low latency), chi phí hợp lý, và khả năng hỗ trợ **Structured JSON Output** mạnh mẽ. Mô hình được ứng dụng vào 2 chức năng riêng biệt:
